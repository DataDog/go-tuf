...
func (s *ClientSuite) TestUpdateRoots(c *C) {
	var tests = []struct {
		...
		{"testdata/PublishedTwiceWithStaleVersion_root", false, ErrWrongVersion(verify.ErrWrongVersion{Given: 1, Expected: 2}), map[string]int{}},
		// New root update with fast forward attack fails.
		{"testdata/PublishedTwiceForwardVersionWithRotatedKeys_root", false, ErrWrongVersion(verify.ErrWrongVersion{Given: 3, Expected: 2}), map[string]int{}},
